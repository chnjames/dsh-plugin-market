...
| 仍看到 `Web UI running at :3789` | 旧 profile 里还有 `ui.webPort`；从 `cordis.patch.yml` 删掉该字段并重启（本机已不再提供独立面板） |
